#!/usr/bin/env python3
"""Move video files from Ok into Ok/video with verified moves.

Default mode is dry-run. Execute mode hashes the source, copies to a temporary
destination under Ok/video, verifies the destination hash, then removes the
source. Matching JSON sidecars are moved beside the video when present.
"""
...
DEFAULT_VIDEO_ROOT = Path("/mnt/windows/transfer/Ok/video")
ORGANIZED_ROOT_PARTS = ("Photos", "Unfiltered", "_organized_from_ok")
...
def iter_video_files(ok_root: Path, video_root: Path) -> list[Path]:
    videos: list[Path] = []
    for current, dirnames, filenames in os.walk(ok_root):
        ...
        for dirname in dirnames:
            ...
            if is_under(candidate, video_root):
                continue
...
def relative_destination(ok_root: Path, source: Path) -> Path:
    organized_root = ok_root.joinpath(*ORGANIZED_ROOT_PARTS)
    try:
        return source.relative_to(organized_root)
    except ValueError:
        try:
...
def decide(ok_root: Path, video_root: Path) -> list[VideoMoveDecision]:
    decisions: list[VideoMoveDecision] = []
    for source in iter_video_files(ok_root, video_root):
        try:
        ...
        destination = unique_destination(video_root / relative_destination(ok_root, source), digest)
        decisions.append(
            VideoMoveDecision(
                source_path=str(source),
...
def move_sidecars(decision: VideoMoveDecision) -> list[dict[str, str]]:
    moved: list[dict[str, str]] = []
    source_video = Path(decision.source_path)
    destination_video = Path(decision.destination_path)
    for raw_path in json.loads(decision.sidecar_paths or "[]"):
        source = Path(raw_path)
        if not source.exists():
            continue
        if source.name == f"{source_video.name}.json":
            destination = destination_video.with_name(f"{destination_video.name}.json")
        else:
            destination = destination_video.with_suffix(".json")
        try:
...
def summarize(decisions: list[VideoMoveDecision], *, mode: str, ok_root: Path, video_root: Path, execution: dict | None = None) -> dict:
...
def write_outputs(decisions: list[VideoMoveDecision], summary: dict, video_root: Path, stamp: str) -> tuple[Path, Path]:
    manifest_dir = video_root / "_manifests"
    manifest_dir.mkdir(parents=True, exist_ok=True)
    csv_path = manifest_dir / f"move-ok-videos-{stamp}.csv"
    json_path = manifest_dir / f"move-ok-videos-{stamp}.summary.json"
    field_names = list(asdict(VideoMoveDecision("", "", "", "")).keys())
...
def main() -> int:
    ...
    decisions = decide(ok_root, video_root)
    ...
    csv_path, json_path = write_outputs(decisions, summary, video_root, utc_stamp())
