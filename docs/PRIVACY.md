...
- Private beta mobile sync should use Tailscale/HTTPS where available and expose only `/health` and `/mobile/*`. Plain trusted hotspot/LAN HTTP such as `http://<laptop-hotspot-ip>:4821` is development mode. Non-loopback binding requires `PRIVATE_GALLERY_ALLOW_REMOTE_MOBILE=1`, and remote clients are limited to `/health` and `/mobile/*`; desktop/admin routes still return `403`.
...
- With Tailscale Serve, keep the daemon on loopback and expose only `/mobile` and `/health`. The daemon treats Tailscale Serve identity headers as remote clients so desktop routes remain blocked for tailnet-proxied requests.
