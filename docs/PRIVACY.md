...
- Non-loopback daemon binding is rejected unless explicit developer mode is enabled in code or `PRIVATE_GALLERY_ALLOW_REMOTE_MOBILE=1` is set for mobile-only remote access.
...
- Paired Android devices use a one-time desktop pairing token and a beta-scoped bearer token stored in Android secure storage. The daemon persists only the bearer-token hash and supports session listing and revocation without exposing token hashes.
- Private beta mobile and local-web sync should use Tailscale/HTTPS where available and expose only `/health`, static `/local-web/*` assets, and paired `/mobile/*` APIs. Plain trusted hotspot/LAN HTTP such as `http://<laptop-hotspot-ip>:4821` is development mode. Non-loopback binding requires `PRIVATE_GALLERY_ALLOW_REMOTE_MOBILE=1`, and remote clients are limited to `/health`, `/local-web/*`, and `/mobile/*`; desktop/admin routes still return `403`.
- Optional phone-created groups use Supabase only for metadata bootstrap when the app is built with explicit `PRIVATE_GALLERY_SUPABASE_URL` and `PRIVATE_GALLERY_SUPABASE_ANON_KEY` values. The cloud schema stores group names, anonymous membership records, client device ids, optional public keys/capabilities/endpoint hints, hashed invite secrets, and expiry/acceptance timestamps only; originals, thumbnails, OCR text, embeddings, vault keys, desktop pairing tokens, LAN bearer tokens, and media metadata stay out of this service.
- With Tailscale Serve, keep the daemon on loopback and expose only `/mobile`, `/local-web`, and `/health`. The daemon treats Tailscale Serve identity headers as remote clients so desktop routes remain blocked for tailnet-proxied requests.
...
- Sync keys, device-pairing secrets, and mobile bearer tokens.
